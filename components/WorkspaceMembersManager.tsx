...
export function WorkspaceMembersManager({
  ...
  const profileById = useMemo(
    () => new Map(profiles.map((memberProfile) => [memberProfile.id, memberProfile])),
    [profiles],
  );
  ...
  async function addMember(event: React.FormEvent<HTMLFormElement>) {
    ...
    const data = (await response.json()) as ApiMemberResponse;

    if (!response.ok || !data.member) {
      setError(data.message || "No se pudo añadir el miembro.");
    ...
    if (data.profile) {
      setProfiles((current) => [
        data.profile!,
        ...current.filter((memberProfile) => memberProfile.id !== data.profile!.id),
      ]);
    }

    setEmail("");
    setRole("member");
    setFeedback("Miembro añadido al workspace.");
    setPendingAction(null);
    router.refresh();
  }
    ...
    <section className="surface rounded-md p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Gestiona quién puede trabajar dentro de este workspace y qué rol tiene cada persona.
          </p>
        </div>
      ...
      {!workspace && (
        <div className="mt-5 rounded-md border border-dashed border-[#d8f3dc] bg-[#faf9f6] p-5 text-sm leading-6 text-slate-600">
          Todavía no tienes un workspace activo.
        </div>
      )}

      {workspace && !canManageMembers && (
        <div className="mt-5 rounded-md border border-[#d8f3dc] bg-[#faf9f6] p-5">
          <p className="font-semibold text-[#2d6a4f]">Gestión disponible en Empresa</p>
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Puedes ver el equipo, pero añadir miembros, cambiar roles o quitarlos queda reservado para plan Empresa o
            administradores.
          </p>
        </div>
      )}
      ...
      {workspace && canManageMembers && (
        <form onSubmit={addMember} className="mt-5 grid gap-3 rounded-md border border-[#d8f3dc] bg-[#faf9f6] p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
            ...
            <button className="focus-ring btn-primary px-4 py-3 text-sm" type="submit" disabled={pendingAction === "add"}>
              {pendingAction === "add" ? "Añadiendo..." : "Añadir miembro"}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="mt-5 grid gap-3">
        {members.length === 0 ? (
          <div className="rounded-md border border-dashed border-[#d8f3dc] bg-[#faf9f6] p-5 text-sm leading-6 text-slate-600">
            Todavía no hay miembros asociados a este workspace.
          </div>
        ) : (
          members.map((member) => {
