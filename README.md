...
## Mi catálogo y solicitudes a medida

DocuGen separa tres niveles de creación para que el usuario no se pierda:

- Catálogo base: los tipos documentales principales que aparecen en `/generar` y `/catalogo`.
- A medida: documentos que el usuario pide con texto libre desde Crear o Asistente.
- Mi catálogo: tipos personales guardados por cada usuario a partir de documentos a medida o del historial.

El foco de producto es que cada usuario pueda guardar, duplicar y ajustar sus propios tipos reutilizables sin depender de una cola pública de aprobación.

Capacidades actuales:

- Guardar documentos generados en `Mi catálogo` desde el resultado o el historial.
- Reutilizar tipos personales desde `/generar` sin mezclarlos con el historial completo.
- Editar campos, descripción e instrucciones avanzadas de cada tipo personal.
- Recalcular campos sugeridos con IA cuando el tipo guardado necesita mejorar su formulario.
- Duplicar tipos personales para crear variantes sin perder el original.
- Revisar solicitudes a medida en `/admin` con estados internos y notas, sin convertir cada petición en una cola pública.
