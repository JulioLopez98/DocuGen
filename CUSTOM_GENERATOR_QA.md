...
- Iniciar sesion con un usuario Free y otro Pro.
...
3. Si el usuario es Free, confirmar que aparece la pantalla de bloqueo Pro y el CTA a `/precios`.
4. Repetir el flujo con usuario Pro.
5. Enviar una solicitud valida:
...
6. Confirmar que aparece el resultado en la misma pantalla.
7. Confirmar que el documento incluye aviso de borrador generado con IA.
8. Probar `PDF`, `TXT`, `Copiar` y `Word Pro`.
9. Probar `Volver a generar` y confirmar que crea una nueva version visible.
10. Abrir `Ver en historial` y confirmar que el detalle carga correctamente.
...
1. Como usuario Free, intentar llamar a `POST /api/custom-generate`.
2. Confirmar respuesta `pro_required`.
...
- Free ve el modo a medida bloqueado con CTA a Pro.
- Pro puede generar documentos a medida y mantiene exportacion Word.
